...
from colorama import Fore, init
os.system('cls')
system('title Black Tool V2 - Internet Check')
colorama.init()
url = "https://github.com"
Write.Print(f"[{datetime.now()}]Internet check...", Colors.blue_to_green, interval=0.000)
time.sleep(1)
try:
    response = requests.get(url)  # Get the responce from the url
    Write.Print(f"""
    [$]Connection Verified Successfully!""", Colors.black_to_red, interval=0.000)
    time.sleep(1)
except requests.exceptions.ConnectionError:
    # Tell the user
    colorama.init()
    system('title Black Tool V2 - Connection Failed')
    os.system('cls')
    Write.Print(f"""[>]You are not connected to internet, check your connection and try again.""", Colors.blue_to_red, interval=0.000)
    time.sleep(1.5)
    Write.Input(f"""
[>]Press ENTER to exit:""", Colors.red_to_white, interval=0.000)
    os.system('cls')
    system('title Connection Failed, Exiting...')
    Write.Print(f"""[>]Exiting...""", Colors.green_to_blue, interval=0.000)
    time.sleep(1.5)
    exit()  # Exit program
try:
...
def loadbar(iteration, total, prefix='', suffix='', decimals=1, lenght=100, fill='>'):
    ...
    Write.Print(f'\r{prefix} |{bar}| {percent}% {suffix}', Colors.purple_to_blue, interval=0.000, end='\r')
    if iteration == total:
        print()
...
os.system('cls')
system('title Black Tool V2 - Soul Tools')
Write.Print(f"""
██████╗░██╗░░░░░░█████╗░░█████╗░██╗░░██╗  ████████╗░█████╗░░█████╗░██╗░░░░░  ██╗░░░██╗██████╗░
██╔══██╗██║░░░░░██╔══██╗██╔══██╗██║░██╔╝  ╚══██╔══╝██╔══██╗██╔══██╗██║░░░░░  ██║░░░██║╚════██╗
██████╦╝██║░░░░░███████║██║░░╚═╝█████═╝░  ░░░██║░░░██║░░██║██║░░██║██║░░░░░  ╚██╗░██╔╝░░███╔═╝
